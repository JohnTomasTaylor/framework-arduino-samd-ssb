...
void printCanRegisters(Can* can) {
  if (can == CAN0)
    Serial.println("CAN0: ");
  else
    Serial.println("CAN1: ");
  Serial.print("CCCR = ");
  Serial.println(can->CCCR.reg, HEX);
  Serial.print("NBTP = ");
  Serial.println(can->NBTP.reg, HEX);
  Serial.print("ECR = ");
  Serial.println(can->ECR.reg, HEX);
  Serial.print("PSR = ");
  Serial.println(can->PSR.reg, HEX);
  Serial.print("IR = ");
  Serial.println(can->IR.reg, HEX);
  Serial.print("GFC = ");
  Serial.println(can->GFC.reg, HEX);


}
#define PRINT_CAN_REGISTERS(...) printCanRegisters(__VA_ARGS__)
#else
#define DEBUG_PRINT(...) ((void)0)
#define DEBUG_PRINTLN(...) ((void)0)
#define PRINT_CAN_REGISTERS(...) ((void)0)
#endif
...
int CANSAME5x::begin(long baudrate) {
  ...
  DEBUG_PRINT("tx_function = ");
  DEBUG_PRINTLN(tx_function);
  DEBUG_PRINT("rx_function = ");
  DEBUG_PRINTLN(rx_function);
  ...
  instances[_idx] = this;
  DEBUG_PRINT("______ CAN init _____");
  PRINT_CAN_REGISTERS(hw);
...
void CANSAME5x::dumpRegisters(Stream &out) {
  out.print(hw->CCCR.bit.INIT);
}
...
CANSAME5x can0(PIN_CAN0_TX, PIN_CAN0_RX);
